...
export async function createTravelPlan(
  ...
  const result = client.query(query,[userId,title,destinationCity,destinationCountry,startDate,endDate,budget]);
...
export async function updateTravelPlan(
  ...
  const result = client.query(query,[planId,title,budget]);
  return (await result).rows[0];
}
